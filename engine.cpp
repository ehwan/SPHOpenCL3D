...
void engine_t::load_opencl()
{
  ...
  nonpressure_force = cl::Buffer( context, CL_MEM_READ_WRITE, maxN*sizeof(ehfloat3) );
  pressure_force = cl::Buffer( context, CL_MEM_READ_WRITE, maxN*sizeof(ehfloat3) );
...
#define MAX_NEIGHBORS 120 
  ...
  kernels.assume_neighbor_count =
    decltype(kernels.assume_neighbor_count)( program, "assume_neighbor_count" );
...
void engine_t::set( param_t &param )
{
  ...
  if( debug )
  {
    ...
    std::cout << "bound : (" << minbound.s[0] << ", " << minbound.s[1] << ", " << minbound.s[2] << ")x("
      << maxbound.s[0] << ", " << maxbound.s[1] << ", " << maxbound.s[2] << ")\n";
    std::cout << "gridsize : (" << gridsize.s[0] << ", " << gridsize.s[1] << ", " << gridsize.s[2] << ")\n";
  }
...
void engine_t::prefix_sum( cl::Buffer &buf, int size )
{
  ...
  queue.enqueueReadBuffer( buf, CL_TRUE, 0, sizeof(cl_int)*(size+1), B.data() );
  int x = 0;
  ...
  queue.enqueueWriteBuffer( buf, CL_TRUE, 0, sizeof(cl_int)*(size+1), B.data() );
  /*
  cl_int err;
  kernels.prefix_sum_phase1(
...
void engine_t::grid_sort()
{
  cl::Event event;
  int gs = gridsize.s[0]*gridsize.s[1]*gridsize.s[2];
  queue.enqueueFillBuffer( grid_particlecount, cl_int(0), 0, sizeof(cl_int)*(gs+1), 0, &event );
  event.wait();
  ...
  queue.enqueueReadBuffer( grid_particlecount, CL_TRUE, sizeof(cl_int)*gs, sizeof(cl_int), &N );
...
void engine_t::make_neighbors()
{
  ...
  cl_int MN;
  queue.enqueueReadBuffer( neighbor_count, CL_TRUE, sizeof(cl_int)*N, sizeof(cl_int), &MN );
  // std::cout << MN << "\n";
...
void engine_t::calculate_mass()
{
  ...
  for( int i=0; i<N; ++i )
  {
    if( ff[i] & EH_PARTICLE_STATIC ){ continue; }
